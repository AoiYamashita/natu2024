...
class birdsEyeViewImage:
    ...
    def SearchLine(self,img,k = 3):
        ...
        ret, dst = cv2.threshold(sobel ,30,255,cv2.THRESH_BINARY)
    ...
    def SearchEraser(self,img,k = 3):
        ...
        contours = [x for x in contours if cv2.contourArea(x) > 100 and cv2.contourArea(x) < img.shape[0]*img.shape[1]*0.3]
        maskImg = img.copy()*0
        ...
        for i in contours:
            ...
            if w_ == 0 or h_ == 0 or h_*w_/2 > area or h_*w_ > img.shape[0]*img.shape[1]*0.3:
                continue
...
class ImgProcess(Node):
    ...
    def cb(self,data):
        ...
        try:
            Edges = self.bevi.SearchLine(img[y0:y1,x0:x1],5)
